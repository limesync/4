...
public class GiveCoinsCommand implements CommandExecutor {
    ...
    public GiveCoinsCommand(MobRealmsCore plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            ...
            replacements.put("usage", "/givecoins <player> <amount>");
            sender.sendMessage(plugin.getMessageManager().getPrefix() + " " +
        ...
        sender.sendMessage(plugin.getMessageManager().getPrefix() + " " +
                plugin.getMessageManager().getMessage("coins.added", replacements));
