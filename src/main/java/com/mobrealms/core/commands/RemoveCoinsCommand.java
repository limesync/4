...
public class RemoveCoinsCommand implements CommandExecutor {
    ...
    public RemoveCoinsCommand(MobRealmsCore plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            ...
            replacements.put("usage", "/removecoins <player> <amount>");
            sender.sendMessage(plugin.getMessageManager().getPrefix() + " " +
        ...
        sender.sendMessage(plugin.getMessageManager().getPrefix() + " " +
                plugin.getMessageManager().getMessage("coins.removed", replacements));
